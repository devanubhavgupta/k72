...
const Navbar = () => {
    ...
    return (
        <div className='flex fixed top-0 w-full items-start justify-between z-4'>
            <div className='lg:p-4 p-2 cursor-pointer'
                onClick={() => {
                    navigate('/');
                    setNavOpen(false);
                }}
            ...
            <div onClick={() => {
                ...
                className='lg:h-13 h-12 lg:w-[16vw] w-48 relative bg-black cursor-pointer'>
                <div ref={navGreenRef} className='bg-[#D3FD50] transition-all h-0 w-full cursor-pointer absolute top-0'></div>
                <div className='relative h-full flex flex-col lg:gap-1.5 gap-0.5 items-end justify-center lg:px-5 px-8'>
